...
const DATA = [
    ...
    },
    {
        state: "Punjab",
        code: "PB",
        districts: ["Amritsar", "Barnala", "Bathinda", "Faridkot", "Fatehgarh Sahib", "Fazilka", "Ferozepur", "Gurdaspur", "Hoshiarpur", "Jalandhar", "Kapurthala", "Ludhiana", "Mansa", "Moga", "Muktsar", "Nawanshahr", "Pathankot", "Patiala", "Rupnagar", "Mohali", "Sangrur", "Tarn Taran"]
    },
    {
        state: "Rajasthan",
        code: "RJ",
...
async function main() {
    console.log("Starting India Seeding...");

    // 1. Seed District 0 (Global) - Manual ID forcing
    try {
        await prisma.$executeRaw`SET SESSION sql_mode='NO_AUTO_VALUE_ON_ZERO'`;
        // Ensure State 1 exists (sample) just in case
        await prisma.state.upsert({ where: { id: 1 }, update: {}, create: { id: 1, state_name: "Others", code: "OTH" } });

        await prisma.$executeRaw`INSERT IGNORE INTO tbl_districts (id, state_id, district_name, created_at, updated_at) VALUES (0, 1, 'All Districts', NOW(), NOW())`;
        console.log("District 0 ensured.");
    } catch (e) { console.log("Dist 0 error (might exist):", e); }

    // 2. Loop States
    // We already have some states (TN id:1 or 23?). Let's just upsert by name to discover ID or create.
    // Actually, user wants all states. We should probably truncate or just add missing. 
    // Truncating is risky if relations exist. Upsert is safer.

    for (const s of DATA) {
        // Find or create State
        const state = await prisma.state.findFirst({ where: { state_name: s.state } });
        let stateId = state?.id;

        if (!state) {
            const newState = await prisma.state.create({
                data: { state_name: s.state, code: s.code }
            });
            stateId = newState.id;
            console.log(`Created State: ${s.state} (${stateId})`);
        } else {
            // Update code if needed
            if (state.code !== s.code) await prisma.state.update({ where: { id: state.id }, data: { code: s.code } });
        }
        ...
        if (stateId) {
            for (const dName of s.districts) {
                const district = await prisma.district.findFirst({
                    where: { state_id: stateId, district_name: dName }
                });
                ...
                if (!district) {
                    ...
                    // console.log(`  + District: ${dName}`); // Noisy
                }
            }
        }
    }

    console.log("India Seeding Completed.");
}
