...
/**
 * Logs a warning for requests that take longer than the threshold.
 */
...
/**
 * Aborts requests that exceed a timeout (default 30 s) to prevent hanging connections.
 */
export const requestTimeout = (req: Request, res: Response, next: NextFunction) => {
  ...
  req.setTimeout(TIMEOUT_MS, () => {
    if (!res.headersSent) {
      logger.error(`Request timeout: ${req.method} ${req.originalUrl} exceeded ${TIMEOUT_MS}ms`);
...
/**
 * Returns middleware that sets Cache-Control headers for public GET requests.
